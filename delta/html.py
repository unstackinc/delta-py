...
def link(root, op):
    if isinstance(op['attributes']['link'], dict):
        ...
        if (
            not current_or_new_tab
            or (smart_url and (smart_url.lower().startswith('coupon:') or smart_url.lower().startswith('cart:')))
        ):
...
def append_op(root, op):
    ...
    if isinstance(text, str) and text:
        smart_url = (op.get('attributes') or {}).get('link') or {}
...
def render(delta, method='html', pretty=False, restrict_header=None):
    ...
    for line, attrs, index in delta.iter_lines():
        ...
        if restrict_header and wcount > 50 and not restrict_header_inserted:
